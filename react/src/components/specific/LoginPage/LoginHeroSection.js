...
const LoginHeroSection = () => {
    ...
    const {  account, login } = useAccount(); // Get the login function from AccountContext
    const { showToast } = useToast(); // Get the showToast function from ToastContext

    useEffect(() => {
        if (account.user) {
            navigate('/profile'); // Navigate to profile page if already logged in
        }
    }, [account, navigate]);
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (success) {
                navigate('/profile'); // Navigate to homepage on successful login
            } else {
                showToast(message, 'danger'); // Show error message using ToastContext
            }
        } catch (error) {
            showToast('An error occurred while logging in', 'danger'); // Handle any unexpected errors
        }
    };

    return (
        <div className="hero-login-banner" style={{backgroundImage: `url(${outdoorsy})`}}>
                        ...
                        <div className="row gy-5 justify-content-center">
                            ...
                            <div className="col-12 col-lg-5 d-flex align-items-center">
                                <div className="d-flex gap-3 flex-column w-100">
                                    <span className="btn bsb-btn-2xl btn-outline-dark rounded-0 d-flex align-items-center">
